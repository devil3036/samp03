// -----------------------------------------------------------------------------
// -----------------------------------------------------------------------------
// Example Filterscript for the new Rusos Building with Elevator
// --------------------------------------------------------------------
...
//
// This script creates the new Rusos building object, removes the
...
// teleport command (/lsb) that teleports you to the Rusos building.
//
// Warning...
...
// Dialog ID for the Rusos elevator dialog
#define DIALOG_ID           (877)

// Position defines
#define X_DOOR_R_OPENED     (735.95398)
#define X_DOOR_L_OPENED     (735.95398)
#define Y_DOOR_R_OPENED     (-1275.13000)
#define Y_DOOR_L_OPENED     (-1275.13000)

#define X_FDOOR_R_OPENED    (735.75398)
#define X_FDOOR_L_OPENED    (735.75398)
#define Y_FDOOR_R_OPENED    (-1275.33000)
#define Y_FDOOR_L_OPENED    (-1275.33000)

#define GROUND_Z_COORD      (2072.36304)     // (33.825077)
#define X_ELEVATOR_POS      (735.95398)
#define Y_ELEVATOR_POS      (-1275.13000)
///CreateDynamicObject(18755, 735.95398, -1275.13000, 2072.36304,   0.00000, 0.00000, 0.00000); // Planta Parking.
...
static Float:FloorZOffsets[3] =
{
    0.0,  // Planta baja.
    127.8269,  // Acceso Principal.
    145.7399,  // HQ Rusos.
};
...
// is unloaded
new Obj_Elevator, Obj_ElevatorDoors[2], Obj_FloorDoors[3][2];
...
new Text3D:Label_Elevator, Text3D:Label_Floors[3];
...
public OnPlayerCommandText(playerid, cmdtext[])
{
	...
	if (strcmp("/lsb", cmdtext, true, 4) == 0)
	{
		...
		GameTextForPlayer(playerid, "~b~~h~Rusos!", 3000, 3);
...
public OnFilterScriptInit()
{
	...
	// Display information in the Server Console
    print("  |--  Rusos Building Elevator created");
    print("  |---------------------------------------------------");
	// Exit here
	return 1;
}
...
public OnFilterScriptExit()
{
	...
	// Display information in the Server Console
    print("  |--  Rusos Building Elevator destroyed");
    print("  |---------------------------------------------------");
				...
				if (i <= 1)
				{
				    ...
				    if (ElevatorState != ELEVATOR_STATE_MOVING)
				    {
				        ...
				        if (ElevatorFloor == i + 1)
				        {
							...
							GameTextForPlayer(playerid, "~n~~n~~n~~n~~n~~n~~n~~y~~h~Rusos Elevator Is~n~~y~~h~Already On This Floor...~n~~w~Walk Inside It~n~~w~And Press '~k~~CONVERSATION_YES~'", 3500, 3);

							// Display chat text message to the player
	                        SendClientMessage(playerid, COLOR_MESSAGE_YELLOW, "* The Rusos elevator is already on this floor... walk inside it and press '{FFFFFF}~k~~CONVERSATION_YES~{CCCCCC}'");
					...
					// Display gametext message to the player
					GameTextForPlayer(playerid, "~n~~n~~n~~n~~n~~n~~n~~n~~g~~h~Rusos Elevator~n~~g~~h~Has Been Called...~n~~w~Please Wait", 3000, 3);
					...
					if (ElevatorState == ELEVATOR_STATE_MOVING)
					{
						...
						format(strTempString, sizeof(strTempString), "* The Rusos elevator has been called... it is currently moving towards the %s.", FloorNames[ElevatorFloor]);
					}
					else
					{
					    ...
					    {
					    	// Format chat text message
							format(strTempString, sizeof(strTempString), "* The Rusos elevator has been called... it is currently at the %s.", FloorNames[ElevatorFloor]);
						}
						else
						{
					    	// Format chat text message
							format(strTempString, sizeof(strTempString), "* The Rusos elevator has been called... it is currently on the %s.", FloorNames[ElevatorFloor]);
						}
					}
...
stock Elevator_Initialize()
{
	...
	Obj_Elevator 			= CreateObject(18755, X_ELEVATOR_POS, Y_ELEVATOR_POS, GROUND_Z_COORD, 0.000000, 0.000000, 0.000000);
	Obj_ElevatorDoors[0] 	= CreateObject(18757, X_ELEVATOR_POS, Y_ELEVATOR_POS, GROUND_Z_COORD, 0.000000, 0.000000, 0.000000);
	Obj_ElevatorDoors[1] 	= CreateObject(18756, X_ELEVATOR_POS, Y_ELEVATOR_POS, GROUND_Z_COORD, 0.000000, 0.000000, 0.000000);
	...
	for (new i; i < sizeof(Obj_FloorDoors); i ++)
	{
	    ...
	    Obj_FloorDoors[i][0] 	= CreateObject(18757, X_ELEVATOR_POS, Y_ELEVATOR_POS - 0.245, GetDoorsZCoordForFloor(i) + 0.05, 0.000000, 0.000000, 0.000000);
		Obj_FloorDoors[i][1] 	= CreateObject(18756, X_ELEVATOR_POS, Y_ELEVATOR_POS - 0.245, GetDoorsZCoordForFloor(i) + 0.05, 0.000000, 0.000000, 0.000000);
...
stock ShowElevatorDialog(playerid)
{
	...
	ShowPlayerDialog(playerid, DIALOG_ID, DIALOG_STYLE_LIST, "Rusos Elevator...", string, "Accept", "Cancel");
