...
// Position defines
#define Y_DOOR_CLOSED       (-1275.1412)
#define Y_DOOR_R_OPENED     Y_DOOR_CLOSED - 1.6
#define Y_DOOR_L_OPENED     Y_DOOR_CLOSED + 1.6

#define GROUND_Z_COORD      (2072.3669)

#define ELEVATOR_OFFSET     (0.05)

#define X_ELEVATOR_POS      (735.9714)
#define Y_ELEVATOR_POS      (-1275.1532)
...
{
    0.0, 		// Car Park 735.9714, -1275.1532, 2072.3669
    127.82,		// Recibidor 735.9714, -1275.1532, 2200.1880
    145.76   	// Planta Única 735.9714, -1275.1532, 2218.1470
};
...
public OnObjectMoved(objectid)
{
	...
	if(objectid == Obj_Elevator)   // The elevator reached the specified floor.
	{
	    ...
	    Label_Elevator	= Create3DTextLabel("{CCCCCC}Presiona '{FFFFFF}~k~~CONVERSATION_YES~{CCCCCC}' para usar el ascensor", 0xCCCCCCAA, X_ELEVATOR_POS - 1.7, Y_ELEVATOR_POS - 1.75, z - 0.4, 4.0, 0, 1);
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    if(dialogid == DIALOG_ID)
    {
        ...
        if(FloorRequestedBy[listitem] != INVALID_PLAYER_ID || IsFloorInQueue(listitem))
            GameTextForPlayer(playerid, "~r~El piso está en espera...", 3500, 4);
		else if(DidPlayerRequestElevator(playerid))
		    GameTextForPlayer(playerid, "~r~Ya solicitaste el ascensor, espera que venga.", 3500, 4);
		else
				...
				if (i <= 1)
				{
				    ...
				    if (ElevatorState != ELEVATOR_STATE_MOVING)
				    {
				        ...
				        if (ElevatorFloor == i + 1)
				        {
	                        ...
	                        SendClientMessage(playerid, COLOR_MESSAGE_YELLOW, "[HQ Rusos] El ascensor está en su piso, entre dentro y presione '{FFFFFF}~k~~CONVERSATION_YES~{CCCCCC}'");
					...
					if (ElevatorState == ELEVATOR_STATE_MOVING)
					{
						...
						format(strTempString, sizeof(strTempString), "[HQ Rusos] El ascensor ha sido solicitado, su piso actual es %s.", FloorNames[ElevatorFloor]);
					}
					else
					{
...
stock Elevator_Initialize()
{
	...
	Label_Elevator = Create3DTextLabel("{CCCCCC}Presiona '{FFFFFF}~k~~CONVERSATION_YES~{CCCCCC}' para usar el ascensor.", 0xCCCCCCAA, X_ELEVATOR_POS - 1.7, Y_ELEVATOR_POS - 1.75, GROUND_Z_COORD - 0.4, 4.0, 0, 1);
	...
	for (new i; i < sizeof(Obj_FloorDoors); i ++)
	{
		...
		format(string, sizeof(string), "{CCCCCC}[%s]\n{CCCCCC}Presiona '{FFFFFF}~k~~CONVERSATION_YES~{CCCCCC}' para solicitar el ascensor", FloorNames[i]);
...
stock ShowElevatorDialog(playerid)
{
	...
	ShowPlayerDialog(playerid, DIALOG_ID, DIALOG_STYLE_LIST, "[HQ Rusos] Ascensor de la facción, eliga su planta", string, "Aceptar", "Cancelar");
