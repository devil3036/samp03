...
public OnObjectMoved(objectid)
{
    ...
    if(objectid == Obj_Elevator)   // The elevator reached the specified floor.
    {
        ...
        Label_Elevator    = Create3DTextLabel("{CCCCCC}Presione '{FFFFFF}~k~~CONVERSATION_YES~{CCCCCC}' para usar el elevador", 0xCCCCCCAA, 1784.9822, -1302.0426, z - 0.9, 4.0, 0, 1);
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    if(dialogid == DIALOG_ID)
    {
        ...
        if(FloorRequestedBy[listitem] != INVALID_PLAYER_ID || IsFloorInQueue(listitem))
            GameTextForPlayer(playerid, "~r~EL PISO YA FUE LLAMADO", 3500, 4);
        else if(DidPlayerRequestElevator(playerid))
            GameTextForPlayer(playerid, "~r~Ya solicito el elevador", 3500, 4);
        else
...
stock Elevator_Initialize()
{
    ...
    Label_Elevator          = Create3DTextLabel("{CCCCCC}Presione '{FFFFFF}~k~~CONVERSATION_YES~{CCCCCC}' para usar el elevador", 0xCCCCCCAA, 1784.9822, -1302.0426, 13.6491, 4.0, 0, 1);
    ...
    for(new i; i < sizeof(Obj_FloorDoors); i ++)
    {
        ...
        format(string, sizeof(string), "{CCCCCC}[%s]\n{CCCCCC}Presione '{FFFFFF}~k~~CONVERSATION_YES~{CCCCCC}' para llamar al elevador", FloorNames[i]);
