/*
 *  Southclaw's Texture Workshop (Version 1.6)
 *      ~ Highlight indexes
...
{
	if(!strcmp(cmdtext, "/editarobjeto"))
	{
	    if(!IsPlayerAdmin(playerid)) return Msg(playerid, YELLOW, "[RCON] Comando no disponible.");
	    EnterEditMode(playerid);
		LoadTextDrawsForPlayer(playerid);
		return 1;
	}
	if(!strcmp(cmdtext, "/salireditar"))
	{
	    if(!IsPlayerAdmin(playerid)) return Msg(playerid, YELLOW, "[RCON] Comando no disponible.");
	    ExitEditMode(playerid);
		return 1;
	}
	if(!strcmp(cmdtext, "/menueditar")) // This is a command for in case your keyboard explodes and only has the 't', '/', 'm', 'e', 'n', and 'u' keys available
	{
	    if(!IsPlayerAdmin(playerid)) return Msg(playerid, YELLOW, "[RCON] Comando no disponible.");
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	if(dialogid == d_MainMenu)
	{
	    if(response)
	    {
			...
			if(listitem == 2)ShowPlayerDialog(playerid, d_TexIdxInput, DIALOG_STYLE_INPUT, "Change Texture", "Enter a material indfex to edit", "Accept", "Cancel");
			...
			if(listitem == 5)ShowPlayerDialog(playerid, d_SaveNameInput, DIALOG_STYLE_INPUT, "Save Material", "Type a name to tag the data in the file:", "Accept", "Back");
			if(listitem == 6)ResetObject(playerid);
			if(listitem == 7)ResetCurrentIdxToOriginal(playerid);
		}
		else return 0;
	}
...
UpdateObjectMaterial(playerid, type=IDX_TYPE_MATERIAL)
{
	...
	if(!type)
	{
		SetPlayerObjectMaterial(playerid, editObj[playerid], tmpIdx, tmpModelId[playerid][tmpIdx], tmpTxdName[playerid][tmpIdx], tmpTexName[playerid][tmpIdx], tmpMatColour[playerid][tmpIdx]);
	}
	else
	{
	    new
	...
	while(iLoop<MAX_INDEX)
	{
	    ...
	    if(tmpIdxType[playerid][iLoop] == IDX_TYPE_TEXT)
	    {
			format(strSave, 256, "SetObjectMaterialText(objectid, \"%s\", %d, OBJECT_MATERIAL_SIZE_%s, \"%s\", %d, %d, %d, %d, %d);\r\n",
				...
				tmpObjFontCol[playerid][iLoop],
				tmpObjBackCol[playerid][iLoop],
